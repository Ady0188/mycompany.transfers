...
public sealed class PrepareCommandHandler : IRequestHandler<PrepareCommand, ErrorOr<PrepareResponseDto>>
{
    ...
    public async Task<ErrorOr<PrepareResponseDto>> Handle(PrepareCommand request, CancellationToken ct)
    {
        try
        {
            _logger.LogInformation("PrepareCommandHandler: AgentId={AgentId}, ExternalId={ExternalId}, ServiceId={ServiceId}, Amount={Amount}, Currency={Currency}",
                request.AgentId, request.ExternalId, request.ServiceId, request.Amount, request.Currency);

            var agent = await _agents.GetByIdAsync(request.AgentId, ct);
            if (agent is null)
            {
                _logger.LogInformation("PrepareCommandHandler: AgentId={AgentId}, agent not found", request.AgentId);
            ...
            {
                _logger.LogInformation("PrepareCommandHandler: existing transfer Id={Id}, Status={Status}", existing.Id, existing.Status);
            ...
            {
                _logger.LogWarning("PrepareCommandHandler: AgentId={AgentId}, ExternalId={ExternalId} service not allowed ServiceId={ServiceId}", request.AgentId, request.ExternalId, request.ServiceId);
            ...
            {
                _logger.LogWarning("PrepareCommandHandler: AgentId={AgentId}, currency not allowed Currency={Currency}", request.AgentId, request.Currency);
            ...
            if (service is null)
            {
                _logger.LogWarning("PrepareCommandHandler: service not found ServiceId={ServiceId}", request.ServiceId);
            ...
            {
                _logger.LogWarning("PrepareCommandHandler: account definition not found AccountDefinitionId={AccountDefinitionId}", service.AccountDefinitionId);
                return AppErrors.Common.Validation($"Определение счёта '{service.AccountDefinitionId}' не найдено.");
            }

            var account = AccountRules.Normalize(request.Account, accountDef);
            ...
            {
                _logger.LogWarning(ex, "PrepareCommandHandler: validation failed");
            ...
            {
                _logger.LogWarning("PrepareCommandHandler: provider not found or disabled ProviderId={ProviderId}", service.ProviderId);
            ...
            {
                _logger.LogWarning("PrepareCommandHandler: currency '{PayoutCurrency}' not available for service", request.PayoutCurrency);
                return AppErrors.Common.Validation($"There not avalable currency '{dstCurrency}' for current service.");
            }

            var fx = await _fxRates.GetAsync(request.AgentId, request.Currency, dstCurrency, ct);
            if (fx is null)
                return AppErrors.Common.Validation($"FX rate not found for {request.Currency}->{dstCurrency}");

            var (rate, asOfUtc) = fx.Value;
            const int srcMinorUnit = 2, dstMinorUnit = 2;
            var creditedMinor = _converter.ConvertMinor(
                request.Amount, srcMinorUnit, dstMinorUnit, rate, service.FxRounding ?? "floor");
            ...
            try
            {
                quote = Quote.Create(total, fee, providerFee, creditedAmount, rate, asOfUtc, TimeSpan.FromMinutes(5), nowUtc);
            }
            catch (DomainException ex)
            {
                _logger.LogWarning(ex, "PrepareCommandHandler: quote create failed");
                return AppErrors.Common.Validation(ex.Message);
            }

            var transfer = Transfer.CreatePrepare(
                request.AgentId, request.TerminalId, request.ExternalId, request.ServiceId,
                (Domain.Transfers.TransferMethod)request.Method, account, request.Amount, request.Currency, request.Parameters, quote);
            ...
            if (provider.IsOnline)
            {
                var providerReq = new ProviderRequest(
                    agent.Id, "prepare", transfer.Id.ToString(), transfer.NumId, transfer.ExternalId,
                    ...
                    service.AllowedCurrencies.First(), service.Name, transfer.Parameters, null, transfer.CreatedAtUtc);

                var providerResult = await _providerService.SendAsync(service.ProviderId, providerReq, ct);

                if (providerResult.Status == OutboxStatus.SUCCESS || providerResult.Status == OutboxStatus.SENDING)
                {
                    transfer.SetReceivedParams(providerResult.ResponseFields);
                    transfer.ApplyProviderResult(
                        _clock.GetUtcNow(), "0", "OK", null, ProviderResultKind.Success, TransferStatus.PREPARED);
                }
